...
class Fsscustomer extends Validate {

    //设置规则
    protected $rule = array(
        'name|成员姓名' => 'max:36',
        'password|密码' => 'require|min:6|alphaDash|token',
        'rpassword|确认密码' => 'require|confirm:password',
        'phone|手机号码' => 'require|unique:fsscustomer|checkPhone',
        'email|电子邮箱' => 'unique:fsscustomer|email',
        'id_card|身份证号码' => 'unique:fsscustomer|checkIdcard',
    );
    //验证场景
    protected $scene = array(
        'alone' => ['name', 'password', 'rpassword', 'email'],
    );

    // 验证手机号码
    protected function checkPhone($value) {
        $result = preg_match('/^(13[0-9]{9})|(18[0-9]{9})|(14[0-9]{9})|(17[0-9]{9})|(15[0-9]{9})$/', $value);
        if (!$result) {
            return '请输入正确的手机号码';
        } else {
            return true;
    ...
    protected function checkIdcard($value) {
        $result = preg_match('/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/', $value);
        if (!$result) {
            return '请输入正确的身份证号码';
        } else {
            return true;
