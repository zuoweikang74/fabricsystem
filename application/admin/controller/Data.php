...
class Data extends Common {
    ...
    public function backups() {
        ...
        $value['data'] = $data;
        $this->view->engine->layout('database');
    ...
    public function export($ids = null, $id = null, $start = null) {
        ...
        if ($Request->isPost() && !empty($ids) && is_array($ids)) { //初始化
            ...
            //备份指定表
            $Database = new \org\util\Database(Session::get('backup_file'), Session::get('backup_config'));
    ...
    public function import() {
        ...
        $value['data'] = $list;
        $this->view->engine->layout('revert');
        return $this->view->assign($value ? : null)->fetch();
    }
    ...
    public function revert($time = 0, $part = null, $start = null) {
        if (is_numeric($time) && is_null($part) && is_null($start)) { //初始化
            ...
            $db = new \org\util\Database($list[$part], [
